...
class EM_Matrix(EM_Count):
    ...
    def init_em_matrix(self):  # Counts, freq, odds, then log odds
    ...
    def freq_matrix_convert_bgs(self):
        self.em_log_odds_matrix[:, 0] = self.em_log_odds_matrix[:, 0] / sum(
            self.em_log_odds_matrix[:, 0])  # Turn background to freq

    def freq_matrix_convert_cols(self):
        col_totals = self.em_log_odds_matrix.sum(axis=0)
        for i in range(1, self.motif_width + 1):
            self.em_log_odds_matrix[:,  # Turn columns to freq
                                    i] = self.em_log_odds_matrix[:,
                                                                 i] / col_totals[i]
