...
def exp_max(user_iter, len_list, len_seq, fasta_file_seq, motif_width, score_matrix_log_odds, em_motifs):
	...
	for i in range(user_iter):
		for j in range(len_list):
			...
			for k in range(last_motif_pos):
				...
				# Convert to characters, match to row and assign as log odds
				score_em_motif = exp_max_score_log_odds(motif_width, list_em_motif, score_matrix_log_odds)
			...
			final_motif[i].append(fasta_file_seq[j][max_pos:(max_pos + motif_width)])
		score_matrix_log_odds = exp_max_matrices(len_list, fasta_file_seq, motif_width, final_max_pos[i])
	...
	# Return an array with the max values and the rest of the values that correspond to them
